...
async function main() {
  ...
  if (args.length === 0) {
    ...
    console.log("Usage: npx hardhat run scripts/createCouponInProject.js --network <network> <projectId>");
    process.exit(1);
  }

  const projectId = args[0]; // First argument is the project ID
  ...
  const couponMetadata = {
    name: "Sample Coupon in Project",
    description: "This is a sample coupon for an existing project",
  ...
  try {
    ...
    try {
      ...
      console.log("Found project:", {
        name: project.name,
        owner: project.owner,
        createdAt: new Date(project.createdAt.toNumber() * 1000).toLocaleString()
      });
    } catch (error) {
      console.error("Error retrieving project. Make sure the project ID exists:", error);
      process.exit(1);
    }
    ...
    console.log("Creating new LazyMint contract in project", projectId, "...");
    const tx = await factory.createLazyMint(
      projectId,        // Existing project ID
      "",               // Project name (empty as we're using an existing project)
